...
=== "准备工作"
    ...
    # 添加 Extformer-MoE 目录到系统环境变量 PYTHONPATH 中
...
=== "模型训练命令"
    ...
    ``` sh
    # ICAR-ENSO 数据预训练模型: Earthformer
    python examples/extformer_moe/extformer_moe_enso_train.py
    # ICAR-ENSO 数据预训练模型: Earthformer + MoE
    python examples/extformer_moe/extformer_moe_enso_train.py MOE.use_linear_moe=true MOE.use_attn_moe=true
    # ICAR-ENSO 数据预训练模型: Earthformer + MoE + RNC
    python examples/extformer_moe/extformer_moe_enso_train.py MOE.use_linear_moe=true MOE.use_attn_moe=true RNC.use_rnc=true
    ```

=== "模型评估命令"

    ``` sh
    # ICAR-ENSO 模型评估: Earthformer
    python examples/extformer_moe/extformer_moe_enso_train.py mode=eval EVAL.pretrained_model_path=./outputs_extformer_moe_pretrain/attn_cuboid-latent/checkpoints/best_model.pdparams
    # ICAR-ENSO 模型评估: Earthformer + MoE
    python examples/extformer_moe/extformer_moe_enso_train.py MOE.use_linear_moe=true MOE.use_attn_moe=true mode=eval EVAL.pretrained_model_path=./outputs_extformer_moe_pretrain/attn_cuboid-latent/checkpoints/best_model.pdparams
    # ICAR-ENSO 模型评估: Earthformer + MoE + RNC
    python examples/extformer_moe/extformer_moe_enso_train.py MOE.use_linear_moe=true MOE.use_attn_moe=true RNC.use_rnc=true mode=eval EVAL.pretrained_model_path=./outputs_extformer_moe_pretrain/attn_cuboid-latent/checkpoints/best_model.pdparams
    ```

| 模型 | 变量名称 | C-Nino3.4-M | C-Nino3.4-WM | MSE(1E-4) |
| :-- | :-- | :-- | :-- | :-- |
| [Earthformer]([MODEL_PATH]) | sst | - | - | - |
| [Extformer-MoE]([MODEL_PATH]) | sst | - | - | - |
